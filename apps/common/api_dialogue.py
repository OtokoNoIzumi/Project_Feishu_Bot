...
def build_dialogue_router(settings: BackendSettings) -> APIRouter:
    ...
    @router.get("/api/dialogues", response_model=List[Dialogue], dependencies=[Depends(auth_dep)])
    ...
    @router.post("/api/dialogues", response_model=Dialogue, dependencies=[Depends(auth_dep)])
    ...
    @router.get("/api/dialogues/{dialogue_id}", response_model=Dialogue, dependencies=[Depends(auth_dep)])
    ...
    @router.patch("/api/dialogues/{dialogue_id}/message", response_model=Dialogue, dependencies=[Depends(auth_dep)])
    ...
    @router.patch("/api/dialogues/{dialogue_id}/messages/{message_id}", response_model=Dialogue, dependencies=[Depends(auth_dep)])
    ...
    @router.get("/api/cards", response_model=List[ResultCard], dependencies=[Depends(auth_dep)])
    ...
    @router.get("/api/cards/{card_id}", response_model=ResultCard, dependencies=[Depends(auth_dep)])
    async def get_card(
        card_id: str,
        service: DialogueService = Depends(get_service)
    ):
    ...
    @router.patch("/api/cards/{card_id}", response_model=ResultCard, dependencies=[Depends(auth_dep)])
