...
不同事件的独立记录和设置一次后默认关联？
比如洗头和头痒，头屑
又比如洗发液用一半，或者月度检查—没结束

关于记录数据的一些思考
1. 结束本身也是一个状态变化，包括洗完澡，梳子变粘手等，用属性和关联而不是数据结构来处理
2. 这些属性当然需要一个识别和建立并维护表格。其中属性的部分在产生记录的时候作为快捷方式写进去，但逻辑字段每次都是取——如果有的话
3. 这样就可以标识哪些是开启事件，哪些是完结，这样AI就能快速定位到开启
4. 数据本身记录完了之后可能需要一个diff和维护的地方，并且可以汇总
5. 关联的逻辑在卡片下方可以加一些勾选，支持多选并分析，这样就可以求出业务来，比如洗澡用了多久——至于不同的类型这个要不要做分支再看，也就是洗头、洗澡、洗头且洗澡这个情况，可以问问AI，这个关联本身也需要一个设定，下次就不用手动设定了，核心就是记录结构化的东西
6.关联的数据至少2个来源，最近新的记录情况，和逻辑上匹配的。这样哪怕第一次记录洗澡和洗完澡，应该也会匹配到
7.记录信息和用信息驱动是两码事，这样头痒和规划下一次主动洗头时间啥的，就好说了，也能看到耗时，比如每天安排时间刷牙
8.关于思考和分析，这部分还是需要依托飞书文档或者markdown，但考虑到diff的问题，markdown应该简单一些，具体要看飞书的验证情况

比如牙齿，牙齿就有很多层，水牙线，牙刷，牙线，舌苔——牙齿的状态和这些都相关，而且推送的时候最好组合起来一起推送，合并成一起，有程度追加

有一些可能不适合开始？比如时间周期长的物料，开始用梳子，开始用洗发水——但或许定义上不是不适合开始，而是不主动匹配——每次都从几百条长期项目里匹配有点蠢。

待完成的问题：
一些多余的变量是局部的，可以先不进一步和constant整合

query这个关联
quick的按钮和布局
这些比较关键

还有一个问题就是另一个已知的时间——过期时间，比如牛奶的保质期，或者红包的到期时间？有这个就可以自然推算出日均来。

编辑模式和聚合数据的缓存是一个较大的冲突问题。
