...
Manages API keys for various services, handling loading from environment variables
and .env files, as well as key rotation and failure handling.
"""
...
class APIKeyManager:
    ...
    def __post_init__(self) -> None:
        ...
        logger.info("APIKeyManager initialized with %d keys.", len(self.keys))
    ...
    def _load_from_dotenv(self, env_vars: List[str]) -> List[str]:
        ...
        # Assume if it exists, it is readable (reliably formatted internal project file).
    ...
    def get_key(self, random_select: bool = True) -> Optional[str]:
        ...
        if not available:
            ...
            logger.warning(
                "All API keys marked as failed. Resetting failure status for new cycle."
            )
    ...
    def mark_failed(self, key: str) -> None:
        ...
        if key in self.keys and key not in self.failed_keys:
            logger.warning(
                "Marking API Key as failed: ...%s", key[-4:] if len(key) > 4 else key
            )
    ...
    def add_key(self, key: str) -> None:
        ...
        if k not in self.keys:
            self.keys.append(k)
            logger.info("Added new API Key: ...%s", k[-4:])
...
# 全局单例实例
# Pylint complains about global statement in get_default_api_key_manager,
# but it's a common pattern. Using a module level variable is fine.
_global_api_key_manager: Optional[APIKeyManager] = None


def get_default_api_key_manager() -> APIKeyManager:
    """获取全局唯一的 APIKeyManager 实例"""
    global _global_api_key_manager  # pylint: disable=global-statement
