...
from ..decorators import card_operation_safe, message_conversion_safe
...
class CardHandler:
    ...
    def __init__(
        ...
        # 设置调试函数
        if debug_functions:
            self.debug_p2im_object = debug_functions.get(
                "debug_p2im_object", noop_debug
            )
        else:
                ...
                return P2CardActionTriggerResponse(
                    {
                        "toast": {
                            "type": "error",
                            "content": f"未找到卡片管理器: {card_config_key}",
                        }
                    }
                )
    ...
    def _convert_card_to_context(self, data) -> Optional[MessageContext]:
        ...
        match action_tag:
            case UIElements.SELECT_STATIC:
                ...
                action_value.update(
                    {FieldNames.OPTION: action_option, FieldNames.TAG: action_tag}
                )
            case UIElements.INPUT:
                # 对于input类型，action.input_value包含用户输入的值
                input_value = (
                    action.input_value
                    if hasattr(action, "input_value")
                    else DefaultValues.EMPTY_STRING
                )
                action_value.update(
                    {
            ...
            case _:
                ...
                if action.name == "design_confirm":
                    ...
                    reverse_field_map = {
                        v: k for k, v in DesignPlanConstants.FORM_FIELD_MAP.items()
                    }
        ...
        legacy_context = MessageContext(
            ...
            metadata={"action_value": action_value, "action_tag": action_tag},
        )

        return legacy_context, New_MessageContext

    @card_operation_safe("管理员卡片操作失败")
    def _handle_admin_card_operation(
        self, result_content: Dict[str, Any], card_operation_type: str, **kwargs
    ) -> Any:
        """
            ...
            case CardOperationTypes.SEND:
                ...
                if not chat_id or not message_id:
                    debug_utils.log_and_print(
                        "❌ 发送管理员卡片缺少chat_id或message_id", log_level="ERROR"
                    )
                    return False, None
            case _:
                pass
        ...
        if not operation_type:
            debug_utils.log_and_print(
                "❌ 缺少业务ID (operation_type)", log_level="ERROR"
            )
        ...
        if not card_config_key:
            debug_utils.log_and_print(
                f"❌ 未找到操作类型对应的卡片配置键: {operation_type}",
                log_level="ERROR",
            )
            if card_operation_type == CardOperationTypes.SEND:
                return False, None
        ...
        if not card_manager:
            debug_utils.log_and_print(
                f"❌ 未找到卡片管理器: {card_config_key}", log_level="ERROR"
            )
            if card_operation_type == CardOperationTypes.SEND:
                return False, None
            return False
        update_toast_type = (
            result_content.get("result_type", "success")
        ...
        return card_manager.handle_card_operation_common(
            card_content=card_manager.build_card(
                admin_confirm_action_data=result_content
            ),
        ...
        def update_card_ui(operation) -> bool:
            ...
            try:
                if not operation.ui_message_id:
                    debug_utils.log_and_print(
                        f"❌ 卡片更新失败: 缺少ui_message_id [{operation.operation_id[:20]}...]",
                        log_level="ERROR",
                    )
                    return False
                ...
                if not card_config_key:
                    debug_utils.log_and_print(
                        f"❌ 卡片更新失败: 未找到操作类型对应的卡片配置键 {operation_type}",
                        log_level="ERROR",
                    )
                    return False
                ...
                if not card_manager:
                    debug_utils.log_and_print(
                        f"❌ 卡片更新失败: 未找到卡片管理器 {card_config_key}",
                        log_level="ERROR",
                    )
                    return False
                ...
                # 调用消息发送器的卡片更新方法
                success = self.sender.update_interactive_card(
                    operation.ui_message_id, card_content
                )

                # 只在失败时记录错误日志
                if not success:
                    debug_utils.log_and_print(
                        f"❌ 卡片更新API失败 [{operation.operation_id[:20]}...]",
                        log_level="ERROR",
                    )
                ...
                debug_utils.log_and_print(
                    f"❌ 卡片UI更新异常: {e} [{operation.operation_id[:20]}...]",
                    log_level="ERROR",
                )
                return False
    ...
    def dispatch_card_response(
        ...
        **kwargs,
        ...
        if not card_manager:
            ...
            return ProcessResult.error_result(f"未找到卡片管理器: {card_config_key}")
        if hasattr(card_manager, card_action):
            return getattr(card_manager, card_action)(
                result, context_refactor, **kwargs
            )
        else:
            debug_utils.log_and_print(
                f"❌ 未支持的动作: {card_action}", log_level="ERROR"
            )
            return ProcessResult.error_result(f"未支持的动作: {card_action}")

    @card_operation_safe("标记B站视频为已读失败")
    def mark_bili_read_in_daily_summary(
        self, context_refactor: MessageContext_Refactor
    ) -> P2CardActionTriggerResponse:
        """
            ...
            return P2CardActionTriggerResponse(
                {
                    "toast": {
                        "type": "error",
                        "content": result.error_message or "标记为已读失败",
                    }
                }
            )
    ...
    @card_operation_safe("标记B站视频为已读失败")
    def mark_weekly_action_accepted(
        self, context_refactor: MessageContext_Refactor
    ) -> P2CardActionTriggerResponse:
        """
        直接处理标记周报行动为已读事件，调用业务层方法
        因为没有特地新建daily_summary的卡片模块，所以直接在这里处理。
        ...
        weekly_record = routine_business.load_weekly_record(user_id)
        action_id = context_refactor.content.value.get("action_id", "")
        week_str = action_id.split("_")[0]
        id_num = int(action_id.split("_")[1]) + 1
        current_week_actions = (
            weekly_record.get("weekly_record", {})
            .get(week_str, {})
            .get("strategic_action_suggestions", [])
        )
        for action in current_week_actions:
            if action.get("id", "") == action_id:
                action["accepted"] = not action.get("accepted", False)
                break

        # 保存更新后的周报
        routine_business.save_weekly_record(user_id, weekly_record)

        return P2CardActionTriggerResponse(
            {
                "toast": {
                    "type": "success",
                    "content": f"已更新行动{id_num}为{'采纳' if action['accepted'] else '拒绝'}",
                }
            }
        )
