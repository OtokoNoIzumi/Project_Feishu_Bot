...
class BilibiliCardManager(BaseCardManager):
    ...
    def handle_generate_new_card(
        self, _route_result: RouteResult, context: MessageContext_Refactor, **_kwargs
    ) -> None:
        """
        ...
        if not video_data.get("success", False):
            self.sender.send_feishu_message_reply(
                context, video_data.get("error_message", "")
            )
            return

        self.handle_card_operation_common(
    ...
    def mark_bili_read(self, context: MessageContext_Refactor):
        """
        ...
        Returns:
            P2CardActionTriggerResponse: 卡片更新或提示的响应
        """
        # 1. 校验依赖服务
        ...
        if video_index_int == 0:
            ...
            pageid = cached_video_data["additional_videos"][video_index_int - 1][
                "pageid"
            ]

        # 4. 标记为已读
        if not notion_service.mark_video_as_read(pageid):
            return self.generate_bubble_response("标记为已读失败", "error")
        ...
        if cached_video_data:
            if video_index_int == 0:
                ...
                cached_video_data["additional_videos"][video_index_int - 1][
                    "is_read"
                ] = True
                cached_video_data["additional_videos"][video_index_int - 1][
                    "is_read_str"
        ...
        # 6. 无缓存数据可用于更新时，仍返回成功提示，保持返回类型一致
        return self.generate_bubble_response(Messages.VIDEO_MARKED_READ)
