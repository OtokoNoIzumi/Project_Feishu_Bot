...
class IntentProcessor:
    ...
    def __init__(self, llm_client, model_name: str, config_path: str = None):
        """
        初始化意图处理器

        Args:
            llm_client: Gemini客户端实例
            model_name: 模型名称
    ...
    def _build_stage2_prompt(self, user_input: str, determined_intent: str) -> Optional[str]:
        """构建第二阶段参数提取提示词"""
        if determined_intent not in self.intents:
            return None

        intent_config = self.intents[determined_intent]
        schema_desc = json.dumps(intent_config['stage2_parameters'], indent=2, ensure_ascii=False)

        prompt_parts = [
            f"# 角色：智能助手参数提取专家，当前处理的意图是{determined_intent}",
            "",
            "# 任务：",
    ...
    def extract_parameters_stage2(self, user_input: str, determined_intent: str) -> Dict[str, Any]:
        """执行第二阶段参数提取"""
    ...
    def process_input(self, user_input: str, confidence_threshold: int = None) -> Dict[str, Any]:
        """完整的两阶段意图处理流程"""
        debug_utils.log_and_print(f"🚀 开始两阶段意图处理: '{user_input[:50]}...'", log_level="INFO")
        ...
        # 确定主要意图
        primary_intent, intent_confidence = self.determine_primary_intent(stage1_result, confidence_threshold)
        ...
        if "error" in stage2_result:
            ...
            final_result["parameters"] = stage2_result.get("parameters", {"original_input": user_input})
        else:
            final_result["parameters"] = stage2_result.get("parameters", {})

        debug_utils.log_and_print(f"🎯 意图处理完成: {primary_intent} (置信度: {intent_confidence})", log_level="INFO")
        debug_utils.log_and_print(f"🎯 意图处理明细: \n{final_result}", log_level="INFO")
