...
| 优先级 | 编号 | 问题描述 | 涉及范围 | 状态 |
|--------|------|----------|----------|------|
| **S** | BUG-001 | 点击"重新分析"后保存的数据变空 `{}` | 前端 `collectEditedData` 或状态管理 | 🔴 搁置（待复现） |
| **A** | BUG-002 | diet analyze 的 `occurred_at` 没正确处理，保存时被覆盖为当前时间 | 前端 `saveRecord` + 后端 storage | 🔴 待验证 (需重启后端) |
...
| 优先级 | 编号 | 功能描述 | 涉及范围 | 状态 |
|--------|------|----------|----------|------|
| **A** | FEAT-001 | P3 进度图表：当日摄入 vs 目标的可视化 | 前端 ECharts 集成 | ⬜ 待开发 |
| **A** | FEAT-003 | 失败一键重试机制（避免重新输入） | 前端 Error UX | ⬜ 待开发 |
| **B** | FEAT-002 | 营养标签产品名下拉选择（从 product library 选择） | 前端 UI + API | ⬜ 待开发 |
| **B** | OPT-001 | 优化响应超时(60s)与请求耗时日志 | 后端 Server | ⬜ 待开发 |
| **B** | REFACTOR-001 | dashboard.js 拆分重构（2290行→模块化） | 前端架构 | ⬜ 待规划 |
