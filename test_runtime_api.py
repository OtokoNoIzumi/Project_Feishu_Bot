...
class RuntimeAPIValidator:
    ...
    def _test_bilibili_api(self):
        ...
        print("\n📺 测试B站API")
        try:
            result = self.app_controller.api_trigger_bilibili_update(['favorites'])
            if result['success']:
    ...
    def _test_audio_api(self):
        ...
        print("\n🎤 测试音频API")
        try:
            result = self.app_controller.api_generate_tts("运行时API测试")
            if result['success']:
    ...
    def _test_image_api(self):
        ...
        print("\n🎨 测试图像API")
        try:
            result = self.app_controller.api_generate_image("API测试小猫")
            if result['success']:
    ...
    def _test_scheduler_api(self):
        ...
        try:
            ...
            if result['success']:
                ...
                # 测试添加临时任务
                add_result = self.app_controller.api_add_scheduled_task(
                    "runtime_test_task", "23:59", "daily_schedule"
                )
                if add_result['success']:
                    print(f"   ✅ 添加测试任务成功")

                    # 立即删除测试任务
                    remove_result = self.app_controller.api_remove_scheduled_task("runtime_test_task")
